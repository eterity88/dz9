...
plt.plot(x, b * x + a, 'g')
print(f'математическим методом\n\
y = a + x*b: coef (b)={b}, intercept (a)={a}\n')
plt.title('Поиск решения Математическим методом y = a + b*x')
plt.show()
...
plt.plot(x, b1_2 * x, 'r')
print(f'матричным методом\n\
С интерсептом: coef={b1_1}, intercept={b0_1}\n\
Без интерсепта: coef={b1_2}, intercept={0}\n')
plt.title('Поиск решения матричным методом с интерсептом и без')
plt.show()
...
# вывод результата
print(f'через LinearRegression()\n\
С интерсептом: coef={beta1}, intercept={const1}\n\
Без интерсепта: coef={beta2}, intercept={const2}')
# отобразим исходные точки на графике
plt.scatter(x,y)
plt.title('Поиск решения через LinearRegression() с интерсептом и без')
plt.show()